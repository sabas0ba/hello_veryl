...
for prog in tfwrite tfimage; do
    if output=$(bash software/build_demo.sh "$prog" psram 2>&1); then
        echo "ERROR: $prog psram placement was accepted" >&2
        exit 1
    fi
    case "$output" in
        *"overlaps its receive buffer"*) ;;
        *) echo "ERROR: unexpected $prog psram rejection: $output" >&2; exit 1 ;;
    esac
done
