...
| 最大周波数 | 36.03 MHz（27 MHz 制約に対して PASS） |

LUT の主な消費源はレジスタファイルとみられる．32 x 32 bit を組合せ 2 ポートで
読むため 32:1 の 32 bit マルチプレクサが 2 組必要になる．BSRAM 2 面へ移せば
LUT を大きく減らせる（読み出しが 1 サイクル遅れるが多サイクル FSM なら
状態を 1 つ足すだけで吸収できる）．段階 7 で既存ペリフェラルと同居させる際の
最初の削減候補とする．
...
| 項目 | 内容 | 対応 |
| --- | --- | --- |
| LUT 予算 | 既存 Top が 77%，TopRv 単独で 65%．同居はできない | 段階 7 で (1) レジスタファイルの BSRAM 化，(2) Fat32Reader・TfImageDemo・PsramMemtest のソフトウェア移行 の順に削減する |
