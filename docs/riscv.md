...
レジスタファイルは 32 x 32 bit．x0 は読み出し 0 固定・書き込み無効とする．
BSRAM（`$std::ram`）を 2 面（read/write ポート）で使う案と，フリップフロップで
持つ案があるが，DFF の空き（4158）に対して 992 bit は小さく，読み出し 2 ポートを
1 サイクルで取れる利点からフリップフロップ実装を採る．

## メモリマップ（案）

| 範囲 | サイズ | 内容 |
| --- | --- | --- |
| `0x0000_0000`–`0x0000_3FFF` | 16 KB | BSRAM（命令 + データ）．ブートコードと常駐ルーチン |
| `0x1000_0000`–`0x103F_FFFF` | 4 MB | PSRAM ch0（AXI4-Lite ブリッジ経由） |
| `0x2000_0000`–`0x2000_00FF` | — | MMIO: UART / LED / タイマ |
| `0x2000_0100`–`0x2000_01FF` | — | MMIO: TF カード（ブロックリード） |
| `0x2000_0200`–`0x2000_02FF` | — | MMIO: フレームバッファ制御（ImageScanout の有効化・基底） |

BSRAM は 26 ブロック中 23 が空いている（約 51 KB）ため，16 KB は 8 ブロック程度で収まる．
命令とデータを同一メモリに置く（フォンノイマン）が，多サイクル FSM では
