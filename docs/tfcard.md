...
PSRAM サブシステムと統合した Top（`feature/integrate-tfcard`）を Tang Nano 9K へ
書き込み，UART 出力で確認した．
...
- `TF OK` は TfCtrl の初期化完走（CMD0/CMD8/ACMD41/CMD58）を示す
- 続く行はルート最初の `*.TXT` の内容で，Fat32Reader によるマウント・
  ディレクトリ照合・クラスタチェーン追跡・CMD17 ブロックリードが
  実カード上で通ることを示す（L1 の自作モデルとは独立した確認）
- 使用カードは 1 枚のみ．カード個体差（残課題「カード相性」）は未確認のまま
...
| 項目 | 内容 | 対応 |
| --- | --- | --- |
| 実機確認（L4） | デモ構成で実施済み（下記「実機結果」） | **解決（2026-08-23）**: 統合 Top（PSRAM + TF デモ）で初期化・FAT32 読み出しとも成功．spike 単体構成での確認は不要となった |
